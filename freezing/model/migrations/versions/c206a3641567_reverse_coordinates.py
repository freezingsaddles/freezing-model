...
def reverse_coordinates():
    ...
    results = conn.execute(
        "select ride_id, AsWkt(start_geo) as start_geo, AsWkt(end_geo) as end_geo from ride_geo"
    )
    for row in results:
        ...
        print(
            "({} -> {}), ({} -> {})".format(
                row["start_geo"], new_start_wkt, row["end_geo"], new_end_wkt
            )
        )

        conn.execute(
            "update ride_geo set start_geo=ST_GeomFromText(%s), end_geo=ST_GeomFromText(%s) where ride_id=%s",
            [new_start_wkt, new_end_wkt, row["ride_id"]],
        )
